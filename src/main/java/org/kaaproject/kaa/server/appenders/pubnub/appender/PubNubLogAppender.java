...
public class PubNubLogAppender extends AbstractLogAppender<PubNubConfig> {
	...
	static Map<String, String> schemaNames = new HashMap<>();
	 ...
	 *
	 * @param appender
	 *            the metadata object that contains useful info like application
	 *            token, tenant id, etc.
	 * @param configuration
	 *            the configuration object that you have specified during
    ...
    public void doAppend(LogEventPack logEventPack, RecordHeader header, LogDeliveryCallback listener) {
    	...
    	if (!closed) {
            try {
                ...
                if (!schemaNames.containsKey(logEventPack.getLogSchema().getSchema()))
                {
                	Schema schema = new Schema.Parser().parse(logEventPack.getLogSchema().getSchema());
                	schemaNames.put(logEventPack.getLogSchema().getSchema(), schema.getName());
                }
                
                String channel = MessageFormat.format("{0}-{1}", schemaNames.get(logEventPack.getLogSchema().getSchema()),logEventPack.getLogSchema().getVersion()); 
                LOG.debug("[{}] saving {} objects", getApplicationToken(), dtos.size());
                if (!dtos.isEmpty()) {
                	this.logEventDao.publish(channel, dtos);
